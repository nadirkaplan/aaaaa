'''
For spreading and migration speed investigations
Set the vertex number to be nnum = 16
change gama_sub for different viscosities
define kl_ary for different substrate stiffness (elastic/long-term stiffness)
change K_a for different additional stiffness 
'''
import math as m
import numpy as np
import User_functions_mR as usf
import Traction_mR as T
import Check_mtraction as C
from random import random 
from datetime import date
import pdb

def migration_simulator(uniform, K_a, K_l, gama_sub, nnum):

    fvr_out = 1; #output vs, vr, vp, fsub ... for postprocessing
    dt_min = 200/120/K_l/15;  
    dt = min(0.0015, dt_min);  #  step size 
    Nsteps = int(round(5000/dt/1e4)*1e4) ;  #  time steps
    ...
    fname = 'Biphasic0'+'_Uni'+str(uniform)+'_Kl'+str(round(kl_id*100)/100) +'_Ka'+str(round(ka_id*100)/100)+'_eta'+str(round(gama_id*100)/100) ;
    ...
    Step_inv_no = 0;      steady_step_num = np.zeros([200]);  

    for i in range(Nsteps-1):
        if i%10000<1e-3:
                print('Nsteps=', i);  
                Area_avg = np.mean(Area[int(nuc_step*0.5):nuc_step]);  # print('Area_avg', Area_avg);    
        ...
        if Rac_cyto_diff < Rac_cyto_tol: 
            ...
            if zeta > 0.0:
               for j in range(nnum):
                   ...
                   fa = C.check_faval(alpha, epsilon, ks, kc, fc, ncj, nmj, fm, v0, K_a, K_l, K_clutch, gama_sub, F_p) ;
                   fa_bond[j] = fa;
        ...
        while disp_tol>1e-5 and iteration < 2:
            ...
            for j in range(nnum):
                ...
                Pf_iter = 0;  fdiff = 1.0;   ro_diff = 1.0;   ff_0 = 0;   rro_0 = 0;  Pf_imax = 5; 
                ...
                while (abs(fdiff) > 1e-6  or   abs(ro_diff) > 1e-6) and Pf_iter< Pf_imax:
                    ...
                    xc_xs=T.update_disps(xs0, xc0, Ro_ary, nc[j], K_clutch, gama_sub, K_a, K_l, F_stall, F_p, v0, dt) ;
    ...
    write_to_file_ary = np.array([np.amax(K_a)*nnum/16, np.amax(K_l)*nnum/16,  np.amax(gama_sub)*nnum/16, avg_vel, Area_avg, aspect_avg, Fsub_diff_avgx, Fsub_diff_avgy, Fsub_diff_avg]);
