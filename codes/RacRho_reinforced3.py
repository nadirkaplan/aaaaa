'''
For visco-taxis study
Set the vertex number to be nnum = 16
change G_grad for different viscosity gradient
define K_l for long term stiffness at x,y = 0
define K_a for additional stiffness at x,y = 0
'''
...
def migration_simulator(uniform, duro, K_s, K_a, K_l, gama_sub, K_grad, G_grad,  opt, nnum, dd):
    ...
    shift_distx = np.random.randint(5, 75); 
    shift_disty = np.random.randint(5, 35); 
    Rac_cyto_tol = 1e-6; 
    Rcell = float(5.0); 
    R_nuc0 = float(2.0); 
...
K_l = 0.1*16/nnum ;
K_a = 2.9*16/nnum ;
gama_sub = 1.0*16/nnum ;

uniform = int(0); #1 for refinement study 1; 2 refinement study 2;

K_grad = float(0.0/100*16/nnum) ;

G_grad = float(20.0/80*16/nnum) ;
