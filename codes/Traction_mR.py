...
'''
Backward Euler to update fraction/probablity of closed bond ---  this is for the staggered solver
'''
...
'''
Backward Euler to update fraction/probablity of closed bond ---  this is for the Newton's solver
'''
def bond_rates(alpha, eps, ks, kc, fa, fcr, f, fc):
    if fa > fcr:
        kon = alpha * np.exp(eps*(fa-fcr))  # + f
    else:
        kon = alpha 
    koff = ks * np.exp(f) + kc * np.exp(-f/fc)
    return (kon, koff)
...
'''
def update_disps(xs0, xc0, rho_ary, nc, Kc, eta, Ka, Kl, Fstall, F_p, v0, dt):
    rho = rho_ary[0];
    kon = rho_ary[1];
    koff = rho_ary[2];
...
'''
Newton's solver --- obtaining ro, xc, xs in monolithic scheme
'''
def Jacobian_res(alpha, eps, fa, fcr, ro, xr, xs, ro0, xr0, xs0, kr0, kc0, fc, nc, Kc, eta, Ka, Kl, Fstall, Fck, v0, dt):
    f = np.float64(Kc*(xr - xs));
    if f>100: print('too large f', f)
    [kon, koff]  = bond_rates(alpha, eps, kr0, kc0, fa, fcr, f, fc)
